...
.defines(function () {
    ...
    EntityWeapon = ig.Box2DEntity.extend({
        size: { x: 32, y: 24 },
        zIndex: 300,
        ...
        gravityFactor: 0,
        density: 0.1,
        categoryBits: ig.global.COL_PLAYER_WEAPON,      // collsion type weapon
        maskBits: ~ig.global.COL_BULLET_LOCAL || ~ig.global.COL_BULLET_NET, //Weapons dont currently get hit by bullets
        entType: "weapon",
        ...
        init: function (x, y, settings) {
            ...
            if (!ig.global.wm) {
                ...
                newFilterData.groupIndex = -9;//-9 For weapon and tank chassis
                ...
                if (this.active) {
                    ...
                    if (this.equipped) {
                        ...
                        if (this.buttoned) {//Need to play the button animation on HUD as well
                            ...
                            if (this.behavior == 2) {
                                if (ig.input.state("rightButton") || ig.input.state("rightButton")) {
                            ...
                            if (this.reloading) {
                                ...
                                //Aim Weapons and fire!
                                if (this.behavior == 2) {//Manual Fire Mode
                                    if (ig.input.state("leftButton") && this.group == 1) {
                                        this.aim(ig.game.mousePointer);
                                    } else if (ig.input.state("rightButton") && this.group == 2) {
                                        this.aim(ig.game.mousePointer);
                                    } else {
                                        
                                    }
                                    
                                }else if (this.behavior == 1) {//Hold Fire
            ...
            if (currentBodyAngle > targetAngle + 3 || currentBodyAngle < targetAngle - 3) {//Give 3 deg range
                ...
                if (this.rofTimer.complete()) {                    
                    if (this.wptype == "projectile") {
                        if (this.behavior == 2) {
                            if ((ig.input.state("rightButton") && this.group == 2) || (ig.input.state("leftButton") && this.group == 1)) {
                                this.shoot(target);
        ...
        shoot: function (target) {
            ...
            var scoreIndex = ig.game.getScoreIndexByRemoteId(ig.game.player.remoteId);
            if (ig.global.scores != undefined) {
                if (ig.global.scores.length > 0) {
                    ig.global.scores[scoreIndex].shotsfire = ig.global.scores[scoreIndex].shotsfire + 1;
                }
            }
            ...
            for (var p = 0; p < this.projectileCount; p++) {
                ...
                console.log("bulletcollision",bullet.categoryBits,bullet.categoryBits);
        ...
        effectOnHit: function () {

        },
        effectOnReload: function () {
